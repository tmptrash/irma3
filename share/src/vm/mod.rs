...
impl VM {
    ...
    fn atom_mov(&mut self, mut atom: Atom, core: &mut Core) -> bool {
        ...
        while !stack.empty() {                                            // before while, stack should have >= 1 atom
            ...
            if self.energy < 0 { return false }                           // no energy to continue
            // checks atoms around if they have bond to moved except already moved---------------------------------------------
            if dir % 2 == 0 {                                             // this is speed optimization trick
                for d in DIR_MOV_DISCONNECTED[dir as I] {                 // go through all near atoms
                    let atom = wrld.get_dir_atom(offs, d);
                    if is_atom(atom) {
                        let rev_dir = get_vm_dir(atom);
                        if rev_dir == DIR_REV[d as I] || get_type(atom) == ATOM_IF && get_dir2(atom) == DIR_REV[d as I] {
                            stack.push(wrld.get_offs(offs, d));           // if near atom has a bond with current
                        }
                    }
                }
            } else {
                for d in &DIR_MOV_DISCONNECTED[dir as I][0..3] {          // go through all near atoms
                    let atom = wrld.get_dir_atom(offs, *d);
                    if is_atom(atom) {
                        let rev_dir = get_vm_dir(atom);
                        if rev_dir == DIR_REV[*d as I] || get_type(atom) == ATOM_IF && get_dir2(atom) == DIR_REV[*d as I] {
                            stack.push(wrld.get_offs(offs, *d));
                        }
                    }
                }
            }
            ...
            if get_type(atom) == ATOM_IF {                                // if atom has additional then bond
                ...
                if dir0 != DIR_NO {                                       // there is no near atom
                    ...
                    if dir1 == DIR_NO {                                   // near atom is to far, will add it later
                        ...
                        dir0 = DIR_REV[dir0 as I];                        // get near atom's dir to moved atom
                        a  = wrld.get_atom(o);                            // near atom
                        if get_dir2(a) == dir0 && get_type(a) == ATOM_IF { // near atom has a bond with moved
                            dir1 = DIR_NEAR_ATOM[dir0 as I][dir as I];    // final dir of near atom
                            set_dir2(&mut a, dir1);
                            wrld.set_atom(o, a, &core.io);
                        }
        ...
        if a != ATOM_EMPTY {
            if has_vm_bond(a) {                                           // update VM offs of current atom
    ...
    /// try to create if/then bond for if atom.
    ///
    ...
    fn atom_if(&mut self, atom: Atom, core: &mut Core) -> bool {
        ...
        // runs else scenario
        if has_vm_bond(atom) && is_atom(core.vm_data.world.get_dir_atom(self.offs, get_vm_dir(atom))) {
...
mod tests {
    ...
    fn check_atoms(offs: &Vec<(Offs, Atom)>, world: &World, size: usize) {
        for o in 0..size {
    ...
    #[test]
    fn test_two_atom_mov8() {
        let (cfg_file, core) = init(1);
        let pvms = unsafe{ &mut (*(core as *mut Core)).vms };
        let pvmdata = unsafe{ &mut (*(core as *mut Core)).vm_data };
        let pio = unsafe{ &mut (*(core as *mut Core)).io };
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b1000_1110_0001_1110; // if
        let atom1 = 0b0011_1111_0100_0000; // mov
    ...
    #[test]
    fn test_two_atom_mov9() {
        let (cfg_file, core) = init(1);
        let pvms = unsafe{ &mut (*(core as *mut Core)).vms };
        let pvmdata = unsafe{ &mut (*(core as *mut Core)).vm_data };
        let pio = unsafe{ &mut (*(core as *mut Core)).io };
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0010_0001_0100_0000; // mov
        let atom1 = 0b1000_1110_0001_1110; // if
        let atom2 = 0b1001_1110_0011_1110; // if

        // atoms:     [m]
        //        [i]=[i]
        //             v
        pvms.add(VM::new(100, 1));
        set_atoms(&vec![(1, atom0), (10, atom1), (11, atom2)], &mut pvmdata.world, pio, 100);
        pvms.data[0].atom_mov(atom0, pcore);
    ...
    fn test_two_atom_mov10() {
        ...
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0110_1110_0000_0000; // spl
        let atom1 = 0b0010_0000_1100_0000; // mov
    ...
    fn test_fix() {
        ...
        let pio = unsafe{ &mut (*(core as *mut Core)).io };
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0100_0000_1001_1000; // fix
        let atom1 = 0b0110_0000_0000_0000; // spl
        let atom2 = 0b0110_0000_0000_0000; // spl

        // atoms:    [s][s]
        //        [f]
        pvms.add(VM::new(100, 10));
        set_atoms(&vec![(10, atom0), (1, atom1), (2, atom2)], &mut pvmdata.world, pio, 100);
    ...
    fn test_fix1() {
        ...
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0100_0000_1001_1000; // fix
        let atom1 = 0b1000_1110_0000_0000; // spl
        let atom2 = 0b0110_0000_0000_0000; // spl
    ...
    fn test_fix2() {
        ...
        let pio = unsafe{ &mut (*(core as *mut Core)).io };
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0100_0000_1111_1000; // fix
        let atom1 = 0b0110_0000_0000_0000; // spl
    ...
    fn test_spl() {
        ...
        assert_eq!(pvms.data[0].get_offs(), 1);
        assert_eq!(pvms.data[0].get_energy(), 100 + pcore.cfg.atoms().spl_energy);
    ...
    fn test_spl2() {
        ...
        let pio = unsafe{ &mut (*(core as *mut Core)).io };
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0110_0000_1111_1000; // spl
        let atom1 = 0b1001_1110_0111_1110; // if

        // atoms: [s]-[i]
        pvms.add(VM::new(100, 0));
        set_atoms(&vec![(0, atom0), (1, atom1)], &mut pvmdata.world, pio, 100);
        pvms.data[0].atom_spl(atom0, pcore);
        check_atoms(&vec![(0, atom0), (1, 0b1001_1100_0111_1110)], &pvmdata.world, 100);
        assert_eq!(pvms.data[0].get_offs(), 0);
        assert_eq!(pvms.data[0].get_energy(), 100 + pcore.cfg.atoms().spl_energy);

        pvms.data[0].atom_spl(atom0, pcore);
        check_atoms(&vec![(0, atom0), (1, 0b1001_1100_0111_1100)], &pvmdata.world, 100);
        assert_eq!(pvms.data[0].get_offs(), 0);
        assert_eq!(pvms.data[0].get_energy(), 100 + pcore.cfg.atoms().spl_energy * 2);
    ...
    fn test_spl3() {
        ...
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0110_0000_1111_1000; // spl
        let atom1 = 0b1000_0000_0111_1110; // if

        // atoms: [s]-[i]
        pvms.add(VM::new(100, 0));
        set_atoms(&vec![(0, atom0), (1, atom1)], &mut pvmdata.world, pio, 100);
        pvms.data[0].atom_spl(atom0, pcore);
        check_atoms(&vec![(0, atom0), (1, 0b1000_0000_0111_1100)], &pvmdata.world, 100);
        assert_eq!(pvms.data[0].get_offs(), 0);
        assert_eq!(pvms.data[0].get_energy(), 100 + pcore.cfg.atoms().spl_energy);
    ...
    fn test_if() {
        ...
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0010_1110_1100_0000; // mov >
        let atom1 = 0b1000_1110_1111_1110; // if
        let atom2 = 0b0111_1110_0000_0000; // spl

        // atoms: [s]-[i]
        pvms.add(VM::new(100, 1));
        set_atoms(&vec![(0, atom0), (1, atom1), (2, atom2)], &mut pvmdata.world, pio, 100);
        pvms.data[0].atom_if(atom1, pcore);
        check_atoms(&vec![(0, atom0), (1, atom1), (2, atom2)], &pvmdata.world, 100);
        assert_eq!(pvms.data[0].get_offs(), 0);
        assert_eq!(pvms.data[0].get_energy(), 100 - pcore.cfg.atoms().if_energy);
    ...
    fn test_else() {
        ...
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0010_1110_1100_0000; // mov >
        let atom1 = 0b1000_1111_0111_1110; // if
        let atom2 = 0b0111_1110_0000_0000; // spl
    ...
    fn test_if_broken() {
        ...
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0010_1110_1100_0000; // mov >
        let atom1 = 0b1000_1111_0101_1110; // if
    ...
    fn test_if_broken1() {
        ...
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b0010_1110_1100_0000; // mov >
        let atom1 = 0b1001_1111_0101_1110; // if
    ...
    fn test_if_broken2() {
        ...
        let pio = unsafe{ &mut (*(core as *mut Core)).io };
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b1001_1111_0101_1110; // if
    ...
    fn test_mixed() {
        ...
        let pio = unsafe{ &mut (*(core as *mut Core)).io };
        let pcore = unsafe{ &mut *(core as *mut Core) };
        let atom0 = 0b1000_1110_0110_0110; // if
        let atom1 = 0b0011_1110_1100_0000; // mov
        let atom2 = 0b0011_1111_0100_0000; // mov
        let atom3 = 0b1000_1111_0100_1110; // if
        let atom4 = 0b0110_0000_0000_0000; // spl
        let mut energy = 100;

        // atoms: [i]=[m] >
        //        [i]=[m]
        //             v
        pvms.add(VM::new(energy, 10));
        set_atoms(&vec![(10, atom0), (11, atom1), (21, atom2), (20, atom3), (1, atom4)], &mut pvmdata.world, pio, 100);
        pvms.data[0].run_atom(pcore);
        check_atoms(&vec![(10, atom0), (11, atom1), (21, atom2), (20, atom3), (1, atom4)], &pvmdata.world, 100);
        assert_eq!(pvms.data[0].get_offs(), 11);
        energy -= pcore.cfg.atoms().if_energy;
        assert_eq!(pvms.data[0].get_energy(), energy);

        pvms.data[0].run_atom(pcore);
        check_atoms(&vec![(11, 0b1000_1110_0110_1110), (12, atom1), (21, atom2), (20, 0b1000_1111_0100_1110), (1, atom4)], &pvmdata.world, 100);
        assert_eq!(pvms.data[0].get_offs(), 11);
        energy -= pcore.cfg.atoms().mov_energy * 2;
        assert_eq!(pvms.data[0].get_energy(), energy);
        ...
        pvms.data[0].run_atom(pcore);
        check_atoms(&vec![(11, 0b1000_1110_0110_1110), (12, atom1), (21, atom2), (20, 0b1000_1111_0100_1110), (1, atom4)], &pvmdata.world, 100);
        assert_eq!(pvms.data[0].get_offs(), 21);
        energy -= pcore.cfg.atoms().if_energy;
        assert_eq!(pvms.data[0].get_energy(), energy);
